...
export const getDeliveryCharges = () => adminRequest("/delivery-charges");
export const saveDeliveryCharge = slab => adminRequest(`/delivery-charges${slab.id ? `/${slab.id}` : ""}`, { method: slab.id ? "PUT" : "POST", body: JSON.stringify(slab) });
export const deleteDeliveryCharge = id => adminRequest(`/delivery-charges/${id}`, { method: "DELETE" });
export const calculateDeliveryCharges = (distanceKm, weightKg) => request("/delivery-charges/calculate", { method: "POST", body: JSON.stringify({ distanceKm, weightKg }) });
